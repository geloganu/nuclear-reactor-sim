      program chicago_pile_1
            COMMON IX,ANUMBE
            real:: coord(3),vcoord(3), nenergy(32000),csection(32000)
            IX=99999
            do i=1,31882
                  nenergy(i)=0
                  csection(i)=0
            ...
            open(1,file='output_fission.out',status='UNKNOWN')
            open(4,file='u235fission.in',status='UNKNOWN')
            open(5,file='tally.out',status="UNKNOWN")
            !open(2,file='exit_energies.out',status='UNKNOWN')
            !open(3,file='unmod_energies.out',status='UNKNOWN')

            do i=1,31882
                  read(4,*) nenergy(i),csection(i)
            end do
...
34          call XSECT(ENE,XELAST,XABSOR)
            ...
            !backscatters if Z<0 
            if(coord(3).LT.0)then
                  backsc=backsc+1.
                  sumback=sumback+csection(I)
                  goto 73
            end if


            !transmitted if Z>depth
            if(coord(3).GE.d)then
                  transm=transm+1.
                  sumtran=sumtran+csection(I)
                  !cross_section = cross_section+csection(I)
                  !thermalized if energy below threshold
                  if(ENE.LT.5e-4)therma=therma+1.

                  !organize energy spectrum for exiting neutrons
                  !write(2,*)ENE
